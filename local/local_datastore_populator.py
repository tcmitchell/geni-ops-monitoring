...
class LocalStoreMemoryPopulator:
    def __init__(self, con, aggregate_id, resource_id, num_inserts, sleep_period_sec, db_templates, event_types, table_str):
        self.con = con;
        ...
        self.table_manager = TableManager.TableManager(self.con, db_templates, event_types)
    ...
    def run_inserts(self):
        ...
        for i in range(self.num_inserts):
            ...
            ins_str = "INSERT INTO " + self.table_str + " VALUES ('" + self.aggregate_id + "', '" + self.resource_id + "'," + str(time_sec_epoch) + "," + str(percent_mem_used) + ");" 
            cur = self.con.cursor()            
            cur.execute(ins_str)
            self.con.commit()
            cur.close()
...
def main():
    ...
    lsmp = LocalStoreMemoryPopulator(con, aggregate_id, resource_id, num_ins, per_sec, db_templates, event_types, table_str)
