...
def info_aggregate_args(agg_id): 
    return query_handler.handle_agg_info_query(con, agg_id, tm.schema_dict["aggregate"])

   
@app.route('/info/node/<node_id>', methods = ['GET'])
def info_node_args(node_id): 
    return query_handler.handle_node_info_query(con, node_id, tm.schema_dict["node"])

    
@app.route('/info/interface/<iface_id>', methods = ['GET'])
def info_interface_args(iface_id): 
    return query_handler.handle_iface_info_query(con, iface_id, tm.schema_dict["interface"])
...
def data(): 
    ...
    return query_handler.handle_ts_data_query(con, filters, tm.schema_dict)
